...
The [XML and Camera Image Download Manual](user-guides-and-manuals/xml-and-camera-image-download-manual.md) describes detailed dynamic message sign (DMS) data that can be accessed from the TravelMdiwest.com website for registered users while the `dmsInfo.csv` web service provides publicly available DMS data to all users:
